    ...
    http::StatusLineParser::parse(http::Request& request, http::Parsor& parsor)
{
	...
	while (!is_finished() && iters < 1'000'000)
	{
		...
		if (m_state == State::START)
		{
			if (ch == ' ')
			{
				request.method = parsor.view().substr(0, parsor.cur_pos());
		...
		else if (m_state == State::METHOD_END)
		{
			if (ch == ' ' || ch == '?')
			{
				request.target = parsor.view().substr(m_method_end + 1, parsor.cur_pos() - m_method_end - 1);
		...
		else if (m_state == State::QUERY)
		{
			if (ch == ' ')
			{
				request.query = parsor.view().substr(m_path_end + 1, parsor.cur_pos() - m_path_end - 1);
		...
		{
			if (ch == '\n' && parsor.view()[parsor.cur_pos() - 1] == '\r')
			{
				// 6 == " HTTP/".length()
				const auto version = parsor.view().substr(
					m_query_end + 6,
					(parsor.view().length() - 2) - (m_query_end + 6)
				);
